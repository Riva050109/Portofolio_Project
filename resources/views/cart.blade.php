            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                ...
                <div class="bg-white/20 px-4 py-2 rounded-full backdrop-blur-sm">
                    <span class="text-sm font-medium">
                        <span class="font-bold">{{ \Cart::getContent()->count() }}</span>
                        {{ \Cart::getContent()->count() === 1 ? 'Item' : 'Item' }}
                    </span>
                </div>
            </div>
        </div>
            ...
            <div class="divide-y divide-gray-100">
                ...
                <div
                    ...
                    <div class="col-span-2 md:col-span-5 flex items-center">
                        ...
                        <div class="min-w-0">
                            <h3 class="font-semibold text-gray-800 text-base md:text-lg truncate">{{ $item->name }}</h3>
                            @if($item->attributes->description)
                            <p class="text-sm text-gray-500 mt-1 line-clamp-1">{{ $item->attributes->description }}</p>
                            @endif
                        </div>
                    </div>
            ...
            <div class="mt-8 bg-gray-50 rounded-xl p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium">Rp {{ number_format(\Cart::getSubtotal(), 0) }}</span>
                    </div>
                    ...
                    <div class="flex justify-between text-blue-600">
                        <span>Diskon</span>
                        <span>- Rp
                            {{ number_format(session('coupon')->type === 'fixed' ? session('coupon')->value : (\Cart::getSubtotal() * session('coupon')->value) / 100, 0) }}</span>
                    </div>
                    @endif

                    <div class="flex justify-between">
                        <span class="text-gray-600">Ongkir (5%)</span>
                        <span class="font-medium">Rp {{ number_format(\Cart::getSubtotal() * 0.10, 0) }}</span>
                    </div>
                    ...
                    <div class="border-t border-gray-200 pt-3 mt-3">
                        <div class="flex justify-between">
                            ...
                            <span class="text-xl font-bold text-blue-600">
                                Rp
                                {{ number_format(\Cart::getSubtotal() - (session('coupon') ? (session('coupon')->type === 'fixed' ? session('coupon')->value : (\Cart::getSubtotal() * session('coupon')->value) / 100) : 0) + (\Cart::getSubtotal() * 0.10), 0) }}
                            </span>
                        </div>
                    </div>
                </div>
